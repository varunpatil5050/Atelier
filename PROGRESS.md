...
- **conductor v0: scribe + reviewer, not yet the full agent DAG** — the Coder (scribe) and
  Reviewer roles of doc 07's Planner/Coder/Tester/Reviewer graph exist and interact through
  the room (scribe proposes → reviewer scores → human decides); the Planner (free-form goals)
  and a Tester (runs code in the sandbox) are the remaining roles. The load-bearing shape is
  real (event-sourced runs, agents-as-room-participants, retrieval-grounded prompts, reviewable
  CRDT edits, multi-agent via shared CRDT maps). See the Reviewer row in Phase 3.
...
| Graph artifacts, Qdrant, Debugger/Tester agents, real model, canary deploys | ⬜ | Reviewer done (above); Planner (free-form goals) + a Tester that runs code in the container sandbox are the remaining agent roles |
