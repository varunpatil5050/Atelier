...
- **conductor v0: Planner + Coder + Reviewer, not yet the full DAG** — three of doc 07's four
  roles exist and interact through the room: the Planner decomposes a goal and delegates to
  the Coder (scribe), which proposes; the Reviewer scores; the human decides. The remaining
  role is a Tester (runs code in the sandbox). The load-bearing shape is real (event-sourced
  runs, agents-as-room-participants, retrieval-grounded prompts, reviewable CRDT edits,
  multi-agent orchestration via shared CRDT maps + delegated runs). See the Planner/Reviewer
  rows in Phase 3.
...
| Graph artifacts, Qdrant, Tester/Debugger agents, real model, canary deploys | ⬜ | Planner + Reviewer done (above); a Tester that runs code in the container sandbox is the remaining agent role; wiring the real model is one isolated step behind ModelProvider |
